function [xy,r] = points2circle(A,B,C,P)
...
%     end
% % elseif ~isequal(numel(A),numel(B),numel(C),2), error('The three points should all have 2 elements.')
% % else
% %     P = [A(:) B(:) C(:)] .' ;
% end
% P = [A(:) B(:) C(:)] .' ;
% matrix
M = [1 1 1 1 ;(P(1,1).^2 + P(1,2).^2) P(1,1) P(1,2) 1 ;(P(2,1).^2 + P(2,2).^2) P(2,1) P(2,2) 1 ;(P(3,1).^2 + P(3,2).^2) P(3,1) P(3,2) 1] ;
